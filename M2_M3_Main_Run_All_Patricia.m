...
numGMs      = 80; % *
Sas         = zeros(numGMs,numStruct);
EDPs        = zeros(numGMs,numCase,numStruct);
pd_Cs       = cell(numCase,numStruct);
paramNCs    = zeros(3,numCase,numStruct);
fragSs      = zeros(2001,numDS-1,numCase,numStruct);
DamageProbs = zeros(2001,numDS,numCase,numStruct);
EAL         = zeros(numCase,numStruct);
...
for l=1:numStruct
    ...
    [Sas(:,l), EDPs(:,:,l), pd_Cs(:,l), paramNCs(:,:,l), ...
        ...
        Sa_calc, EAL(:,l), EAL_S(:,l),...
        EAL_NS(:,l),   EAL_C(:,l)] = M2_PBEE_Simple_Patricia(struc, RD_c, Years);
    for k = 1:numCase
...
load('Data/drift_collapse_corr.mat')
...
for m = 1:numPPeVal
    for k=1:numCase
        for l = 1:numStruct
            ...
            Rzz(4,5,k,l,m)   = (1+ELMR+EL_PP_cases(k,l))*...
            ...
            while any(eig(Rzz(:,:,k,l,m))<=0)
               ...
               if R_err==0
                  record_Rzz_err = [record_Rzz_err;k, l]; 
               end
               R_err = R_err + 1;
            end
            R_err = 0;
...
save('Data/Flint_2019_M2_M3_all.mat');
...
for i=1:n_x 
    ...
    if any(i-[4,6,7]==0) % normally distributed with given CV
        J_Ztheta(i,i) = -x_star2(i)/(paramVals(i,1,k,l,m)^2*CV(i))*...
            pdf(fX{i,k,l,m},x_star2(i)) /  pdf(fZ,z_star2(i));
    else %if i==5
...
export(ex_out_pf,'file','examples_pF.txt','Delimiter','tab');
...
for l=1:numStruct
    ...
    for k = 1:numPP % PP types
        ...
        M2_Plot_Fragility(xlims(5,:),Sa_calc,...
            reshape(DamageProbs(:,5,PPcols(l,:),l),length(Sa_calc),3),...
...
gca = axes('Parent',gcf,'YGrid','off','XGrid','off',...
    ...
    'YTick',[0.1, 0.3, 0.5, 0.7],'Ycolor',[0,0,0],...
    'YMinorTick','on','YTickLabel',{'0.1%','0.3%','0.5%','0.7%'},...
